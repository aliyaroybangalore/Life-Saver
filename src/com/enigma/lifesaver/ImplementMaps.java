...
public class ImplementMaps extends FragmentActivity {
	...
	private void getnearbylifesaver() {
		if (CheckNetwork.isInternetAvailable(ImplementMaps.this)) {
			Log.v("lat", lat);
			Log.v("lon", lon);
			UserFunctions userFunction = new UserFunctions();
			jsonarray = userFunction.GetNearbyLifeSaver(PhoneNumber, lat, lon);
	...
	private void ShowNearbyLifeSaver() {
...
public void nolifesaverfound(){
	if(jsonarray.length()==0)
		Toast.makeText(getApplicationContext(), "No LifeSaver found nearby", 5000).show();
	...
	private class SendNotification extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			...
			progressDialog.setMessage("Sending message to LifeSaver, Please wait...");
...
protected void onPostExecute(Void result) {
	super.onPostExecute(result);
	progressDialog.setMessage("Message sent to LifeSaver");
	progressDialog.hide();
}
		}
